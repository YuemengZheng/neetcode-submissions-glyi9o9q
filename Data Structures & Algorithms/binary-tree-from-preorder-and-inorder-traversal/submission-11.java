...
class Solution {
    ...
    private TreeNode helper(int[] preorder, int preStart, int preEnd, Map<Integer, Integer> inorderMap, int inStart, int inEnd){
        ...
        root.left = helper(preorder, preStart + 1, preStart + leftSize, inorderMap, inStart, rootIndex - 1);
        root.right = helper(preorder, preStart + leftSize + 1, preEnd, inorderMap, rootIndex + 1, inEnd);
        return root;
    }
}
