class Solution {
    public List<Integer> findMinHeightTrees(int n, int[][] edges) {
        ...
        peel off the current leaf layer, decrement neighbors' degree. 
        When a neighbor's degree becomes 1, it's the new leaf. 
        Stop when remaining nodes <= 2."
        TC & SC:
        "Time complexity is O(n), each node is processed once. 
        Space complexity is O(n) for the graph and queue."
        */
        if(n <= 2) {
        ...
        int[] degree = new int[n];// leafnode只有一条边联系，所以degree是1
        ...
        int remain = n;
        while(remain > 2){ // check当前层只有2或者1个元素了！！
            int size = q.size();
            remain -= size;
