class MyHashSet {
    ...
    And since 769 is a prime number, it distributes the keys more evenly to reduce collisions. 
    So I'll go with 769.
    Let me illustrate with a small size of 7. 
    If I add key 13, then 13 mod 7 is 6, so it goes into bucket 6. If I call contains(13),
    I go to bucket 6 and find it, so it returns true. 
    Then remove(13) goes to bucket 6 and removes it.

    Time complexity is O(n/k) per operation where n is the number of elements and k is the bucket size. 
    Since k is fixed at 769, it's effectively O(1) on average. 
    Space complexity is O(k + n) for the buckets and stored elements.
    */
    List<Integer>[] set;
    int size = 769;
