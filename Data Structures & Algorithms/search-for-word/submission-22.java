class Solution {
    int[][] DIRS = new int[][]{{0, -1}, {0, 1}, {-1, 0}, {1, 0}};
    public boolean exist(char[][] board, String word) {
        /*
        We try each cell as a possible starting point.
        For each starting point, we use backtracking to match the word.
        At each recursion level, we check if the current cell matches the current character.
        If it matches, we explore four directions to match the next character.
        If we match all characters in the word, we return true. Otherwise, we return false. 

        The time complexity is O(m * n * 4^L), 
        because we try every cell as a starting point, 
        and for each start we may explore up to 4 directions for each character in the word.

        The space complexity is O(L), where L is the length of the word, because the recursion depth is at most L
        */
        int m = board.length;
        int n = board[0].length;
        for(int i = 0; i < m; i++){
            for(int j = 0; j < n; j++){
                if(backtrack(i, j, 0, word, board)){
                    return true;
                }
            }
        }
        return false; 
    }
    private boolean backtrack(int i, int j, int cur, String word, char[][] board){
        if(cur == word.length()){
            return true;
        }

        if(i < 0 || i >= board.length || j < 0 || j >= board[0].length || board[i][j] != word.charAt(cur)){
            return false;
        }
        // 最后一个字符匹配成功后，你仍然需要再递归一次，才能让 index == word.length() 成立。
        // 但如果周围没有合法格子，这次递归根本不会发生，于是会错误返回 false

        char temp = board[i][j];
        board[i][j] = '#';

        for(int[] dir : DIRS){
            int x = dir[0] + i;
            int y = dir[1] + j;
            if(backtrack(x, y, cur + 1, word, board)){
                board[i][j] = temp;
                return true;
            }
        }

        board[i][j] = temp;
        return false;
    }
}
